...
Groups the ouput into 20 minute periods starting with the 20 minute period that the `From` parameter is included in. If no `From` parameter is included, the first ever 20 minute period where there were bidding events is the first period included in the report. If the `To` time parameter does not refer to an even 20 minute period end, the 20 minute period to which it belongs will be included. If the `To` parameter is omitted, all

### Important

The DSP does not store spending snapshots of all periods indefinetely. `_20min` snapshots, for example, are usually removed after 60 days ().
